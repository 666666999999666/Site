...
# Migration and controlled maintenance scripts use one pinned toolchain in
# both the build database gate and the final runtime image.
...
# Every production image is tied to one immutable source revision.  The build
# deliberately fails when the pipeline does not provide a full Git SHA.
ARG APP_RELEASE_SHA
RUN printf '%s' "$APP_RELEASE_SHA" | grep -Eq '^[0-9a-f]{40}$'
