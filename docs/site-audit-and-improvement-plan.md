...
> **实施状态（2026-07-29）**
>
> 本文记录的是改造前审计基线，正文中的 `BUG-*`、`SEC-*`、`ENG-*`、`MANUAL-002` 和适合当前阶段的产品建议均已实施，包括 Todo/Idea 转博客草稿。2026-07-29 的补充收尾还完成了当时的有限双语系统文案、右下角后台入口、Git/镜像源码指纹、自动冒烟、版本化 cron、证书安全安装脚本和灾备文档；其中有限双语方案已被 2026-08-25 的中文单站决定取代，见 `BUG-013`。文中明确列为“暂不建议实施”或达到内容规模后再做的分页、项目详情 CMS 等事项仍按原决策不实施；RSS 已在后续博客优先改造中实现。
>
...
#### BUG-013：英文切换不是完整国际化（有限双语决定已被取代）
...
此前的有限双语决定已被中文单站决定完整取代：

1. 删除英文文案包、语言切换组件和用户界面的英文分支，只保留 `/zh` 公开路由。
2. 保留 `next-intl` 作为单一中文文案基础设施，不迁移为无前缀路由，也不新增双语数据库字段。
3. `/en` 与任何 `/en/**` 永久返回无 `Location` 的 `410 Gone`；未知 locale 返回 404，不能回退为中文页。
4. sitemap、RSS、canonical、Open Graph 和 JSON-LD 只发布 `/zh` URL，不输出英文 hreflang；`robots.txt` 不屏蔽 `/en`，让爬虫能够观察 410。
5. 数据库创作内容保持作者原文，不删除英文文章、技术名词或英文 slug。
...
- 公开页面固定使用中文文案且根 `lang` 为 `zh-CN`，页面中不存在语言切换入口。
- `/en`、尾斜杠、嵌套路径和带查询参数的旧地址均返回 410 且没有 `Location`；`/energy`、`/english` 不误匹配。
- sitemap、RSS、canonical 和 JSON-LD 不含英文站点 URL；未知 locale 返回 404。
- 旧 `NEXT_LOCALE=en` cookie 不能恢复英文界面，数据库结构与作者内容不因本决定被删除或复制。
...
3. 首页个人身份文案。
4. 项目卡片证据增强。
5. BUG-013 删除英文产品模块、固定中文页面语言，并将 `/en/**` 收口为 410。
