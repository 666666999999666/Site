...
**本轮最终收录 34 个可执行条目**，分类如下：

| 分类 | 编号范围 | 数量 | 处理原则 |
|---|---|---:|---|
| 第一优先级缺陷 | `BUG-001`–`BUG-006`、`BUG-019` | 7 | 先修正确性、数据与初始化风险 |
| 核心体验缺陷 | `BUG-007`–`BUG-018`、`BUG-020`–`BUG-021` | 14 | 分批修复，不重写后台 |
...
1. 暂时隐藏语言切换入口。
2. 保留现有路由代码，避免现在进行数据库多语言改造。
3. 至少让 `<html lang>` 与当前 locale 一致。
4. 等有真实英文内容需求时再设计翻译字段。
...
README 声明服务器有每日 PostgreSQL 备份和每周证书检查，但对应脚本不在当前仓库中，本次审计也没有读取服务器 crontab 或备份目录。因此只能确认“文档有声明”，不能确认任务仍在运行或备份可以恢复。

**处理方式**

1. 在服务器上核验 crontab 最近执行时间和最新备份文件大小。
2. 把最新备份恢复到临时数据库，确认文章、项目、设置、Todo 和用户表可读取。
3. 单独核验 `data/uploads` 是否有备份；数据库备份不包含上传文件。
4. 将备份脚本放入受版本控制的运维目录，Secret 继续留在服务器环境中。
5. 记录最近一次成功恢复演练日期。

这是一项人工运维核验，不要求下一位 Agent 在没有服务器权限时伪造完成状态。
...
1. 移入 Header 的图标区或移动菜单。
2. 仅登录后显示固定快捷入口。
3. 如果继续悬浮，为页面底部保留安全空间，并验证手机端不遮挡内容。
