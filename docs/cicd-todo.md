...
网站已成功部署在 http://REDACTED_SERVER_IP，但 CI/CD 自动部署链路尚未打通。
...
3. 或直接部署到服务器：`scp qzsite.tar ubuntu@REDACTED_SERVER_IP:/tmp/` → `docker load` → `docker compose up -d web`
...
4. 验证网站可访问：`curl.exe -s -o NUL -w "%{http_code}" http://REDACTED_SERVER_IP`
5. 验证数据库未重置
